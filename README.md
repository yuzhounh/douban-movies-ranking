...
GitHub Pages 按“分类排行榜、选电影、选剧集”三个一级板块展示数据，并依次使用二级类别和三级标签继续筛选。“全部”会对当前一级板块或二级类别中的作品去重汇总；“分类排行榜”中的三级标签按作品数量从多到少排列，Top 250 作为其中的独立榜单。表格展示排序后的 `id`、`title`、`rating` 和 `rating_count`，点击任意影视条目所在行即可打开对应豆瓣页面，并支持分类内搜索与分页：
